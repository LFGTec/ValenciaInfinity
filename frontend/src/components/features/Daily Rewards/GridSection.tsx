...
import { CheckCircle, Lock, Gift, Star, Trophy, Crown, ChevronDown } from 'lucide-react';
...
function GridSection({ rewards, setToast, onClaim }: GridSectionProps) {
  ...
  const handleClaim = () => {
    if (!todayReward?.available) {
      setToast({ message: "Esta recompensa no está disponible todavía", type: "error" });
      return;
    }
    onClaim(cycleDay);
  };
              ...
              {todayReward ? (
                <>
                  <p className="text-5xl font-black text-foreground mb-2">
                    {todayReward.type === "card" ? "CARTA" : `+${todayReward.reward}`}
                  ...
                  <p className="text-lg text-muted-foreground font-bold">{todayReward.description}</p>
                </>
              ) : (
                <>
                  <p className="text-3xl font-black text-muted-foreground mb-2">Sin recompensa hoy</p>
                  <p className="text-muted-foreground">Este día no tiene recompensa configurada</p>
                </>
              )}
            </div>
            ...
            <div className="flex-shrink-0">
              {todayReward?.claimed ? (
                <div className="flex items-center gap-3 px-8 py-4 bg-green-500/10 border-2 border-green-500/50 rounded-xl">
                  ...
                  <span className="font-black text-green-500 text-lg">RECLAMADO</span>
                </div>
              ) : todayReward?.available ? (
                <button
                  onClick={handleClaim}
                ...
                <div className="flex items-center gap-3 px-8 py-4 bg-muted border-2 border-border rounded-xl">
                  ...
                  <span className="font-black text-muted-foreground text-lg">NO DISPONIBLE</span>
                </div>
              )}
            </div>
          </div>
            ...
            {upcoming.map((reward) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`w-12 h-12 mx-auto mb-3 rounded-full flex items-center justify-center opacity-70 ${REWARD_COLOR[reward.type]}`}>
                ...
                return (
                  ...
                  >
                    <span className={`text-[11px] font-black leading-none ${
                      state === "available" ? "text-vcf-orange"
                      : state === "claimed"   ? "text-green-500"
                    ...
                    <div className={`w-7 h-7 rounded-full flex items-center justify-center ${
                      state === "available" ? REWARD_COLOR[reward.type]
                      : state === "claimed"  ? "bg-green-500"
                      : "bg-muted"
                    }`}>
                      {state === "claimed"
                    ...
                    {(state === "available" || state === "future") && reward.active && (
                      <span className={`text-[9px] font-black leading-none ${
                        state === "available" ? "text-vcf-orange" : "text-muted-foreground"
                      }`}>
                        {reward.type === "card" ? "CARTA" : `+${reward.reward}`}
                      </span>
                    )}
