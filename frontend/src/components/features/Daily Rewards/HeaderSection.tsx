...
/* Funcion para saber que dia se logueo el usuario */
...
/* Calcular que tipo de fan eres */
function getBadge(totalDays: number): {
  label: string;
  color: string;
  next: number | null;
} {
  if (totalDays >= 100)
    return { label: "FAN LEGENDARIO", color: "text-yellow-400", next: null };
  if (totalDays >= 60)
    return { label: "FAN VETERANO", color: "text-purple-400", next: 100 };
  if (totalDays >= 30)
    return { label: "FAN DEDICADO", color: "text-blue-400", next: 60 };
  if (totalDays >= 14)
    return { label: "FAN CONSTANTE", color: "text-green-400", next: 30 };
  if (totalDays >= 7)
    return { label: "FAN ACTIVO", color: "text-vcf-orange", next: 14 };
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
        ...
        <div className="relative rounded-2xl p-8 text-center border-2 border-border bg-card shadow-lg">
          ...
          <div className="text-6xl font-black mb-2 text-foreground">
            {currentStreak}
          </div>
        ...
        >
          ...
          <div className={`text-6xl font-black mb-2 ${numCls()}`}>
            {longestStreak}
          </div>
          <div className={`text-lg font-bold ${labelCls()}`}>
            RACHA MÁS LARGA
          </div>
              ...
              {isBreakingRecord ? (
                ...
                >
                  ...
                  <p className="text-white/80 text-sm">
                    Llevas {currentStreak} días — tu anterior marca era{" "}
                    {longestStreak}
                  </p>
                </motion.div>
              ) : (
            ...
            <div className="bg-card border-2 border-vcf-orange rounded-2xl p-6 shadow-lg">
              ...
              <div className="grid grid-cols-2 gap-4 mb-5">
                <div className="bg-muted/50 rounded-xl p-4 text-center">
                  <p className="text-4xl font-black text-vcf-orange">
                    {cyclesCompleted}
                  </p>
                  <p className="text-xs font-black text-muted-foreground mt-1 tracking-wide">
                    CICLOS DE 14 DÍAS
                  </p>
                </div>
                <div className="bg-muted/50 rounded-xl p-4 text-center">
              ...
              <div className="flex items-center gap-4 border-2 border-vcf-orange/30 rounded-xl p-4">
                ...
                <div>
                  <p className="text-[10px] font-black text-muted-foreground tracking-widest mb-0.5">
                    TU TÍTULO
                  </p>
                  <p className={`text-2xl font-black ${badge.color}`}>
                    {badge.label}
                  </p>
                  {badge.next !== null && (
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Próximo título a los{" "}
